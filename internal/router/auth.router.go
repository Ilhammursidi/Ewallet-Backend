...
func RegisterAuthRouter(router *gin.Engine, db *pgxpool.Pool, rdb *redis.Client) {
	...
	authRouter.PATCH("/enter-pin", middleware.Blacklist(rdb), middleware.VerifyToken, authController.CreatePin)
	authRouter.POST("/logout", middleware.Blacklist(rdb), middleware.VerifyToken, authController.Logout)
